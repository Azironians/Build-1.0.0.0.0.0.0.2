...
//Использованные модули:
  ...
  //1. Изображения:
  var initWindow, loadFrag, fieldWindow, menuWindow, bashWindow,
  ...
  headlinePict, headlineLocMch: PictureABC;
  //7. Кнопки:
 ...
 begin
  initWindow := Picture.Create('HoAData\Windows\WindowInit.bmp');
  ...
  loadFrag := Picture.Create('HoAData\Triggers\TriggerLoadFrag.png'); 
  ...
  menuWindow := Picture.Create('HoAData\Windows\WindowMenu.png'); //Инициализация окна меню.
  beforeWindow := Picture.Create('HoAData\Windows\WindowChampion.png'); //Инициализация окна победителя
  loadFrag.Draw(trunc(844 / 2223 * windowWidth),trunc(1234 / 1434* windowheight),
  fragOffset, trunc(52 / 1434 * windowheight));
  fragOffset := fragOffset + trunc(523 / 2223* windowWidth / 50);
  loadFrag.Draw(trunc(844 / 2223 * windowWidth), trunc(1234 / 1434 * windowheight),
 ...
 ClearWindow(clblack); //Очистка экрана
  ...
  btnExitOff.MoveTo(-10000, -10000);
  btnExitOn.MoveTo(-10000, -10000);
  locMchWindow := Picture.Create('HoAData\Windows\LocMchWindow.png');
  ...
  begin
  triggerWindow := 1;
  locMchWindow.Clear(clBlack);
  headlineLocMch.MoveTo(-10000, -10000);
  btnOffExitLocMch.MoveTo(-10000, -10000);
  btnOnExitLocMch.MoveTo(-10000, -10000);
  btnOffLocMchPlayOnTwo.MoveTo(-10000, -10000);
  btnOnLocMchPlayOnTwo.MoveTo(-10000, -10000);
  menuWindow := Picture.Create('HoAData\Windows\WindowMenu.png');
