...
//Использованные модули:
  ...
  btnExitOff, btnExitOn: PictureABC;
  //8. Процедуры и функции для кнопок:
  
  //8.1 Выключение реагирования мыши:
  ...
  //8.2.3 Оконная процедура меню:
  procedure ProcMenuWindow;
  begin
  ...
  //8.2 Кнопка "Локальный Матч":
  //Реагирование кнопки "Локальный матч" на мышь:
  ...
  //Нажатие на кнопку:
  procedure MouseDownLocMch(x, y, mb: integer);
  begin
  if mb = 1 then
  begin
  if (x > trunc(windowWidth div 2 * 1.4)) and (x < trunc(windowWidth div 9 * 7.93)) and
  (y <= trunc(windowHeight div 10)) then
  //Открытие окна LocMchWindow:
  begin
  menuChoice := 1;
  btnExitOff.Destroy;
  locMchWindow := Picture.Create('LocMchWindow3.png'); //Инициализация окна для локального матча.
  locMchWindow.Draw(0, 0, WindowWidth, WindowHeight); 
  end;
  end;
  end;
  
  //8.2.1 Кнопка "Игра на двоих"
  //Реагирование кнопки на мышь:
  procedure MouseMoveLocMchPlayOnTwo(x, y, mb: integer);
  begin
  if mb = 0 then 
  begin
  if (x > trunc(windowWidth div 2 * 1.4)) and (x < trunc(windowWidth div 9 * 7.93)) and
  (y <= trunc(windowHeight div 10)) then
  begin
  btnOnLocMchPlayOnTwo.MoveTo(trunc(windowWidth div 2 * 1.4), trunc(windowHeight div 92));
  ...
  //9. Оконные процедуры.
  //9.1 Процедура окна меню:

  
  //9.2. Процедура окна "Локальный матч":
  ...
  OnMouseDown := MouseDownExitGame;
  OnMouseDown := MouseDownLocMch;
  end;
  if menuChoice = 1 then 
  begin
  OnMouseMove := MouseMoveLocMchPlayOnTwo;
  OnMouseDown := MouseMoveOut;
  end;
  end;
  
  //10. Процедуры для аудиофайлов:
  procedure MenuMusic;
  begin
   try 
...
devSpotlight := Picture.Create('DEV_Spotlight.png');
 ...
 //Установка шрифта и цвета для написания текста:
  ...
  ProcMenuWindow; //Процедура окна меню
end.
