...
//Использованные модули:
  ...
  //7. Кнопки:
  btnOffLocMch, btnOnLocMch: Picture;
...
procedure MouseInButton(Oldbttn, Newbttn: Picture; dy, dx, width, height, x, y, mb: integer);
begin
if (mb = 0) and (x >= dx) and (x <= dx + width) and (y >= dy) and (y <= dy + height)  then 
begin
Oldbttn := Newbttn;
end;
end;
...
devSpotlight := Picture.Create('DEV_Spotlight.png');
 ...
 //Установка шрифта и цвета для написания текста:
  ...
  btnOffLocMch := Picture.Create('ButtonOffMenuLocMatch.png');
  btnOnLocMch := Picture.Create('ButtonOnMenuLocMatch.png');
  btnOffLocMch.Draw(0,0);
