...
var
  ...
  p, g, bat, bat1, game, star, fon, hell, dam, ult11, ult12, ult21, 
...
begin
  ...
  if hod = 1 then begin
    ...
    if bon = 2 then bonuss21 := bonuss21 + 1; //Экзорцизм
    ...
    if bon = 5 then bonuss51 := 1; //Сокрушающая волна
    if bon = 6 then bonuss61 := 1; //Плюнуть и врезать
  ...
  if hod = 2 then begin
   ...
   windowheight  div 2 + gg.Height div  5, trunc(gg.Height / 863 * 60),  qw22, clwhite);
    ...
    if bon = 5 then bonuss52 := 1; //Сокрушающая волна
...
Window.Caption := 'Heroes of Azironian Build.1.0'; // Название окна
 ...
 second := textabc.Create(-10000, -10000, 40, 'Ход красного игрока', clred);
  ...
  var t := new Timer(100, TimerProc); //таймер
  ...
  while gem = 0 do 
  begin
    dev := PictureABC.Create(-10000, -10000, 'ger1.png'); // Поменять!
    ...
    SetFontColor(clwhite); DrawTextCentered(0, 0, windowWidth, windowHeight, 'Синий игрок, пожалуйста, выбери себе героя');SetFontColor(clwhite);
    t.Start;
    Sleep(2000); vubor := 2;
    bhr.MoveTo(windowwidth - 1, 0);
    dev.MoveTo(-dev.Width, 0);
    ...
    DrawTextCentered(0, 0, windowWidth, windowHeight, 'Красный игрок, пожалуйста, выбери себе героя');g1 := 0;SetFontColor(clwhite);
    t.Start;
    ...
    SetFontColor(clwhite);SetFontSize(40);  DrawTextCentered(0, 0, windowWidth, windowHeight,  'ИГРА НАЧИНАЕТСЯ!');
    t.Start;
    Sleep(2000);
  ...
  if ran=1 then begin  och2 := och2 + u2;
   ...
   if ran=0 then begin  
    ...
    begin
      ...
      hod := 1;
      h := h + 1;if (l1 > 2) and (ul1 = -1) then ul1 := h mod 7;
      if (l1 > 4) and (ul3 = -1) then ul3 := h mod 9;
      if ogl2 = 1 then begin
        ...
        t.Start;
        Sleep(2000);game.draw(0, 0, windowWidth, windowHeight); ogl2 := 0; end else begin
