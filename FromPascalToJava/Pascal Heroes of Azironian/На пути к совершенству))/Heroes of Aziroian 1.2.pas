...
var
  xxx,gemes, bonuss21, bonuss22,qwert, bonuss161, bonuss162, g11, g1, g21, l1,
  l2, h, b,  fir2, fir, ul1, ul2, ul3, ul4, ff, ogl1, ogl2, gem,  vubor, i, her, hod, exx: integer;
  g12, gam: char;
  //Целочисленные бонусы:
  bonuss41, bonuss101, bonuss102,   bonuss42, bonuss51,
  bonuss52, bonuss61, bonuss62, bonuss71, bonuss72, bonuss131, bonuss132,
  bonuss141, bonuss142, bonuss151, bonuss152, bon1, bon2, bon3, bon, bobo: byte;
  //Реальные бонусы:
  bonuss111, bonuss112: real;
  //Активность суперспособностей героев
  is113, is114, is223, is224, is233, is234, is213, 
  is214, is115, is125, is135, is215, is225, is235, is123, is124,
  is133, is134: byte;
  
  qw11, qw12, qw21, qw22, text, bonu: string;
  
  p, g, bat, bat1, game, star, fon, hell, ult11, ult12, ult21, 
  ult22, ult31, ult32, ult13, ult23, ult33, vamp, trol, ad, loading: Picture;
  ran, h1, h2, och1, och2, hmax1, u1, u2, hmax2, hel, hel2, dam: real;
  
  gg, gg2,  lev1, lev2, bhr, dev, lv, ud, plat1, plat2, plus, plus01, plus02,
  plus3, ud1, ud2, ud3, load, Headline, ochki1: PictureABC;
...
procedure Key2(key: char); //ход
begin
  if hod = 1 then begin
    if (Key = '1') then  b := 1; //Атака
    if (Key = '2') then  b := 2; //Лечение
    if (Key = '3') and ((is113 = 1) or (is123 = 1) or (is133 = 1)) then  b := 3; //3 суперспособность
    if (Key = '4') and ((is114 = 1) or (is124 = 1) or (is134 = 1)) then  b := 4; //4 суперспособность
    if (Key = '5') and ((is115 = 1) or (is125 = 1) or (is135 = 1)) then  b := 5; //5 суперспособность
  end else 
  begin
    if (Key = '1') then  b := 1;
    if (Key = '2') then  b := 2;
...
procedure bonus; // Запуск бонусов
begin
  bon1 := Random(1, 16);
  bon2 := Random(1, 16);
  ...
  if hod = 1 then begin
    //Противодействие
    if bon = 9 then begin 
    bon := bobo; // bobo хранит в себе предыдущий бонус
    och1 := och1 + 10;
    end; 
    // Адаптация: + 100% от атаки к вашему здоровью - Upgraded
    if bon = 1 then hmax1 := hmax1 +  u1; 
    //Экзорцизм
    if bon = 2 then bonuss21 := bonuss21 + 1; 
    // Эликсир жизни: Восстанавливает 50 ед. здоровья - Fixed
    if bon = 3 then begin 
    if h1 + 50 > hmax1 then begin
    h1 := hmax1;
    end else begin
    h1 := h1 + 50;
    end;
    he1.Destroy; //Обновление значения здоровья
    str(trunc(h1), qw21);
    he1 := textabc.Create(gg.Width div 2 + 5, windowheight  div 2 +
    gg.Height div  5, trunc(gg.Height / 863 * 60),  qw21, clwhite);
    end;
    if bon = 4 then bonuss41 := 1; //Укрепить броню
    
    if bon = 5 then begin //Сокрушающая волна
    h1 := h1 - 200;
    h2 := h2 -200;
    he1.Destroy; //Обновление значения здоровья
    str(trunc(h1), qw21);
    he1 := textabc.Create(gg.Width div 2 + 5, windowheight  div 2 +
    gg.Height div  5, trunc(gg.Height / 863 * 60),  qw21, clwhite);
    he2.Destroy;str(trunc(h2), qw22); //Обновление значения здоровья
   he2 := textabc.Create(windowWidth - gg.Width div 2 * 3 + gg.Width div 3 * 2 + 5,
   windowheight  div 2 + gg.Height div  5, trunc(gg.Height / 863 * 60),  qw22, clwhite);
    end;  //Сокрушающая волна
    
    if bon = 6 then begin //Плюнуть и врезать
    bonuss61 := 1;
    if h2 < 100 then dam := u1 * 0.25 else dam := u1 * 0.1;
    u1 := u1 + dam;
    da1.Destroy; //Обновление значения атаки
    str(trunc(u1), qw11);
    da1 := textabc.Create(gg.Width div 2 * 3 - gg.Width div 3 + 5,
    windowheight  div 2 + gg.Height div 5, trunc(gg.Height / 863 * 60), qw11, clwhite);
    end;
    
    if bon = 7 then bonuss71 := 1; //Двойной в голову
    if bon = 8 then begin //Поднять Топор: +2 к Атаке - Fixed
    u1 := u1 + 2; //u1 - атака первого игрока
    da1.Destroy; //Обновление значения атаки
    str(trunc(u1), qw11);
    da1 := textabc.Create(gg.Width div 2 * 3 - gg.Width div 3 + 5,
    windowheight  div 2 + gg.Height div 5, trunc(gg.Height / 863 * 60), qw11, clwhite);
    end;
    if bon = 10 then bonuss101 := 3; // Шаг за шагом: Следующие 3 хода вы получаете 110% опыта от атаки - Upgraded
    if bon = 11 then bonuss111 := bonuss111 * 1.02; // Предквкушение: Сложные проценты
    if bon = 12 then begin // Занёрфить: +200% за ульту - Fixed
    ...
    if bon = 13 then bonuss131 := bonuss131 + 1; //Путь мастера
    if bon = 14 then bonuss141 := 20; //Дать жару
    if bon = 15 then bonuss151 := 1; //Казнить
    if bon = 16 then bonuss161 := 1; //На мушке (изменить на Антимагию)
  end;
  // Ход второго игрока
  if hod = 2 then begin
    if bon = 9 then begin //Противодействие
    bon := bobo; // bobo хранит в себе предыдущий бонус
    och2 := och2 + 10;
    end; 
    if bon = 1 then hmax2 := hmax2 +  u2; // Адаптация: + 100% от атаки к вашему здоровью - Upgraded
    if bon = 2 then bonuss22 := bonuss22 + 1; //Экзорцизм
    if bon = 3 then begin // Эликсир жизни: Восстанавливает 50 ед. здоровья - Fixed
    if h2 + 50 > hmax2 then begin
    h2 := hmax2;
    end else begin
    h2 := h2 + 50;
    end;
   ...
   windowheight  div 2 + gg.Height div  5, trunc(gg.Height / 863 * 60),  qw22, clwhite);
    ...
    if bon = 4 then bonuss42 := 1; //Укрепить броню
    if bon = 5 then begin //Сокрушающая волна
    h1 := h1 - 200;
    h2 := h2 -200;
    he1.Destroy; //Обновление значения здоровья
   ...
   windowheight  div 2 + gg.Height div  5, trunc(gg.Height / 863 * 60),  qw22, clwhite);
    ...
    if bon = 6 then bonuss62 := 1; //Плюнуть и врезать
    if bon = 7 then bonuss72 := 1; //Двойной в голову
    if bon = 8 then begin //Поднять Топор: +2 к Атаке - Fixed
    u2 := u2 + 2; //u1 - атака первого игрока
   da2.Destroy;str(trunc(u2), qw12); //Обновление значения атаки
   da2 := textabc.Create(windowWidth - gg.Width div 2 * 3 + 5,
   windowheight  div 2 + gg.Height div  5, trunc(gg.Height / 863 * 60),  qw12, clwhite);
    end;
    if bon = 10 then bonuss102 := 3; // Шаг за шагом: Следующие 3 хода вы получаете 110% опыта от атаки - Upgraded
    if bon = 11 then bonuss112 := bonuss112 * 1.02; // Предквкушение: Сложные проценты
    if bon = 12 then begin // Занёрфить: +200% за ульту - Fixed
    ...
    if bon = 13 then bonuss132 := bonuss132 + 1; //Путь мастера
    if bon = 14 then bonuss142 := 20; //Дать жару
    if bon = 15 then bonuss152 := 1; //Казнить
    if bon = 16 then bonuss162 := 1; //На мушке (изменить на Антимагию)
  end;
  bbb[bon1].moveto(-10000, -10000); //Бонусы уходят на улицу
  bbb[bon2].moveto(-10000, -10000);
  bbb[bon3].moveto(-10000, -10000);
  bobo := bon; //Присвоение предыдущего бонуса
end;


begin
Window.Caption := 'Heroes of Azironian Build.1.0'; // Название окна
  MaximizeWindow; //На весь экран
  p := Picture.Create('pict.bmp');
  p.draw(0, 0, windowWidth, windowHeight);
  
  try //Запуск заставочной музыки:
  ...
  setFontStyle(fsBold);
  DrawTextCentered(0, 0, windowWidth, windowHeight, 'Игра начинается!'); //Создание надписи
  loading := Picture.Create('Loading.png'); //Псевдозагрузка
  xxx := xxx + trunc(523 / 2223 * windowWidth / 50); //Что такое xxx?
  loading.Draw(trunc(844 / 2223 * windowWidth), trunc(1234 / 1434 * windowheight),
  xxx, trunc(52 / 1434 * windowheight));
  xxx := xxx + trunc(523 / 2223 * windowWidth / 50);
  bat   := Picture.Create('Prepare.png'); //Открытие окна подготовки:
  loading.Draw(trunc(844 / 2223 * windowWidth),trunc(1234 / 1434* windowheight),
  xxx, trunc(52 / 1434 * windowheight));
  xxx := xxx + trunc(523 / 2223* windowWidth / 50);
  bat1 := Picture.Create('Map.png'); //Открытие окна для выбора героев:
  loading.Draw(trunc(844 / 2223 * windowWidth), trunc(1234 / 1434 * windowheight),
  xxx, trunc(52 / 1434 * windowheight));
  xxx := xxx + trunc(523 / 2223 * windowWidth / 50);
  game := Picture.Create('vub.bmp'); // Присвоение игрового окна:
  loading.Draw(trunc(844 / 2223 * windowWidth), trunc(1234 / 1434 * windowheight),
  xxx, trunc(52 / 1434 * windowheight));
  xxx := xxx + trunc(523 / 2223 * windowWidth / 50);
  star := Picture.Create('star.jpg'); //Присвоение оглушения для "Оглушителя":
  loading.Draw(trunc(844 / 2223 * windowWidth), trunc(1234 / 1434 * windowheight),
  xxx, trunc(52 / 1434 * windowheight));
  xxx := xxx + trunc(523 / 2223 * windowWidth / 50);
  anim := SpriteABC.Create(-100000, -100000, 'anim.spinf'); //Присвоение анимации:
  loading.Draw(trunc(844 / 2223 * windowWidth), trunc(1234 / 1434 * windowheight),
  xxx, trunc(52 / 1434 * windowheight));
  xxx := xxx + trunc(523 / 2223 * windowWidth / 50);
  loading.Draw(trunc(844 / 2223 * windowWidth), trunc(1234 / 1434 * windowheight),
 ...
 //Загрузка Тем для героев:
  loading.Draw(trunc(844/2223* windowWidth),trunc(1234/1434* windowheight),xxx,trunc(52/1434* windowheight));xxx:=xxx+trunc(523/2223* windowWidth/50);
 ad    := Picture.Create('DEV_Spotlight.png');
  loading.Draw(trunc(844/2223* windowWidth),trunc(1234/1434* windowheight),xxx,trunc(52/1434* windowheight));xxx:=xxx+trunc(523/2223* windowWidth/50);
 ...
 second := textabc.Create(-10000, -10000, 40, 'Ход красного игрока', clred);
  ...
  begin
    ...
    SetFontColor(clblue); DrawTextCentered(0, 0, windowWidth, windowHeight, 'Синий игрок, пожалуйста, выбери героя');SetFontColor(clblue);
    t.Start;
    Sleep(2000); vubor := 2;
    bhr.MoveTo(windowwidth - 1, 0);
    dev.MoveTo(-dev.Width, 0);
    ...
    g12 := '-';SetFontSize(30);
    SetFontColor(clwhite);SetFontSize(50); DrawTextCentered(0, 0, windowWidth, windowHeight, 'Неплохой выбор!');SetFontSize(30);
    t.Start;
    Sleep(1000);bat.draw(0, 0, windowWidth, windowHeight);
    DrawTextCentered(0, 0, windowWidth, windowHeight, 'Красный игрок, пожалуйста, выбери героя');g1 := 0;SetFontColor(clred);
    t.Start;
    Sleep(1500);bat.draw(0, 0, windowWidth, windowHeight);vubor := 2;
    lv.MoveTo((windowwidth - lv.Width) div 2, 0);
    bhr.MoveTo(windowwidth - 1, 0);
    dev.MoveTo(-dev.Width, 0);
    while g21 = 0 do 
    begin
      ...
      begin
        ad.draw(0, 0, windowWidth, windowHeight);
          while (g12 <> #13) and (g12 <> #27) do  begin OnMouseDown := Mousedown3; OnKeyDown := key12; end;
        if (g12 = #13) then begin g21 := 1;bat.draw(0, 0, windowWidth, windowHeight); end;
      ...
      begin
        vamp.draw(0, 0, windowWidth, windowHeight);
       while (g12 <> #13) and (g12 <> #27) do  begin OnMouseDown := Mousedown3; OnKeyDown := key12; end;
        if (g12 = #13) then begin g21 := 2;bat.draw(0, 0, windowWidth, windowHeight); end;
      ...
      begin
        trol.draw(0, 0, windowWidth, windowHeight);
          while (g12 <> #13) and (g12 <> #27) do begin OnMouseDown := Mousedown3; OnKeyDown := key12; end;
        if (g12 = #13) then begin g21 := 3;bat.draw(0, 0, windowWidth, windowHeight); end;
  ...
  ran:=random(0,1); 
  if ran=1 then begin  och2 := och2 + u2;
     bat.draw(0, 0, windowWidth, windowHeight);
   ...
   if ran=0 then begin  och1 := och1 + u1;
     bat.draw(0, 0, windowWidth, windowHeight);
   ...
   if ran=0 then begin  
    ...
    begin
      ...
      if (ran = 1)or (gemes = 1) then begin
      ...
      hod := 1;
      h := h + 1;
      if (l1 > 2) and (ul2 = -1) then ul2 := h mod 7; //было ul1
      if (l1 > 4) and (ul3 = -1) then ul3 := h mod 9;
      if ogl2 = 1 then begin //Оглушение
        star.draw(0, 0, windowWidth, windowHeight);
        SetFontSize(50);
        DrawTextCentered(0, 200, windowWidth, windowHeight, 'Вы оглушены на ход');
        SetFontSize(30);
        t.Start;
        Sleep(2000);
        game.draw(0, 0, windowWidth, windowHeight);
        ...
        bonus;//БОНУС!!!!
        SetFontSize(lev1.Height div 5); DrawTextCentered(0, 5, lev1.Width - 20, lev1.Height div 2, 'Уровень');DrawTextCentered(lev1.Width - 20, 5, lev1.Width, lev1.Height div 2, l1);        DrawTextCentered(windowWidth - lev1.Width, 5, windowWidth - lev1.Width + 20, lev1.Height div 2, l2);DrawTextCentered(windowWidth - lev1.Width + 20, 5, windowWidth, lev1.Height div 2, 'Уровень');
        //Вывод очков на табло:
        if g11 = 1 then begin
        ...
        // Вывод 1-х способностей на экран:
        if ((h mod 5 = 0) and (g11 = 1)) or (is113 = 1) then begin
          is113 := 1;         ult11.Draw(trunc(89 * windowheight / 2156), trunc(1799 * windowheight / 2156), 70, 70) end;
        if ((h mod 5 = 0) and (g11 = 2)) or (is123 = 1) then begin
          is123 := 1;         ult21.Draw(trunc(89 * windowheight / 2156), trunc(1799 * windowheight / 2156), 70, 70) end;
        if ((h mod 5 = 0) and (g11 = 3)) or (is133 = 1) then begin
          is133 := 1;         ult31.Draw(trunc(89 * windowheight / 2156), trunc(1799 * windowheight / 2156), 70, 70) end;
        // Вывод 2-х способностей на экран:
        if ((l1 > 2) and (h mod 7 = ul1) and (g11 = 1)) or (is114 = 1) then begin
          is114 := 1;          ult12.Draw(trunc(3478 * windowheight / 2156), trunc(1799 * windowheight / 2156), 70, 70) end;
        if ((l1 > 2) and (h mod 7 = ul1) and (g11 = 2)) or (is124 = 1) then begin
          is124 := 1;          ult22.Draw(trunc(378 * windowheight / 2156), trunc(1799 * windowheight / 2156), 70, 70) end;
        if ((l1 > 2) and (h mod 7 = ul1) and (g11 = 3)) or (is134 = 1) then begin
          is134 := 1;          ult32.Draw(trunc(378 * windowheight / 2156), trunc(1799 * windowheight / 2156), 70, 70) end; 
        // Вывод 3-х способностей на экран:
        if ((l1 > 4) and (h mod 9 = ul3) and (g11 = 1)) or (is115 = 1) then begin
          is115 := 1;          ult13.Draw(trunc(667 * windowheight / 2156), trunc(1799 * windowheight / 2156), 70, 70) end;
        if ((l1 > 4) and (h mod 9 = ul3) and (g11 = 2)) or (is125 = 1) then begin
          is125 := 1;          ult23.Draw(trunc(667 * windowheight / 2156), trunc(1799 * windowheight / 2156), 70, 70) end;
        if ((l1 > 4) and (h mod 9 = ul3) and (g11 = 1)) or (is135 = 1) then begin
          is135 := 1;          ult33.Draw(trunc(667 * windowheight / 2156), trunc(1799 * windowheight / 2156), 70, 70) end;
