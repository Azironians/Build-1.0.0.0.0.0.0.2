...
var
  ...
  l2, h, b,  fir2, fir, ul1, ul2, ul3, ul4, ff, ogl1, ogl2, gem, is123, is124,
  is133, is134, vubor, i, is113, is114, is223, is224, is233, is234, is213, 
  is214, is115, is125, is135, is215, is225, is235,  her, hod, exx: longint;
  g12, gam: char;
  //Бонусы
  bonuss41, bonuss101, bonuss102, bonuss111, bonuss112, bonuss42, bonuss51,
...
begin
  ...
  if hod = 1 then begin
    if bon = 9 then begin bon := bobo;och1 := och1 + 10; end;
    if bon = 1 then hmax1 := hmax1 +  u1 * 0.15;
    if bon = 2 then bonuss21 := bonuss21 + 1;
    if bon = 3 then begin h1 := h1 + 100;if h1>hmax1 then h1:=hmax1;he1.Destroy;str(trunc(h1), qw21);he1 := textabc.Create(gg.Width div 2 + 5, windowheight  div 2 + gg.Height div  5, trunc(gg.Height / 863 * 60),  qw21, clwhite); end;
    if bon = 4 then bonuss41 := 1;
    if bon = 5 then bonuss51 := 1;
    if bon = 6 then bonuss61 := 1;
    if bon = 7 then bonuss71 := 1;
    if bon = 8 then begin u1 := u1 + 3;da1.Destroy;str(trunc(u1), qw11);da1 := textabc.Create(gg.Width div 2 * 3 - gg.Width div 3 + 5, windowheight  div 2 + gg.Height div 5, trunc(gg.Height / 863 * 60),  qw11, clwhite); end;
    if bon = 10 then bonuss101 := 3;
    if bon = 11 then bonuss111 := bonuss111 + 2;
    if bon = 12 then begin
      if is113 = 1 then begin is113 := 0;och1 := och1 + u1 * 4; end;
      if is123 = 1 then begin is123 := 0;och1 := och1 + u1 * 4; end;
      if is133 = 1 then begin is133 := 0;och1 := och1 + u1 * 4; end;
      if is114 = 1 then begin is114 := 0;och1 := och1 + u1 * 4; end;
      if is124 = 1 then begin is124 := 0;och1 := och1 + u1 * 4; end;
      if is134 = 1 then begin is134 := 0;och1 := och1 + u1 * 4; end;
      if is115 = 1 then begin is115 := 0;och1 := och1 + u1 * 4; end;
      if is125 = 1 then begin is125 := 0;och1 := och1 + u1 * 4; end;
      if is135 = 1 then begin is135 := 0;och1 := och1 + u1 * 4; end;
    end;
  ...
  if hod = 2 then begin
    if bon = 9 then begin bon := bobo;och2 := och2 + 10; end;
    if bon = 1 then hmax2 := hmax2 +  u2 * 0.15;
    if bon = 2 then bonuss22 := bonuss22 + 1;
    if bon = 3 then begin h2 := h2 + 100;if h2>hmax2 then h2:=hmax2;      he2.Destroy;str(trunc(h2), qw22);he2 := textabc.Create(windowWidth - gg.Width div 2 * 3 + gg.Width div 3 * 2 + 5, windowheight  div 2 + gg.Height div  5, trunc(gg.Height / 863 * 60),  qw22, clwhite); end;
